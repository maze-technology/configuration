...
resource "github_branch_protection" "protections" {
  ...
  restrict_pushes {
    push_allowances = each.value.pattern == each.value.default_branch ? [data.github_app.renovate.node_id] : (each.value.pattern == "main" ? [github_team.children["release-engineers"].node_id] : [])
  }
